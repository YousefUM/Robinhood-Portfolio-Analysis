...
def get_current_holdings(transactions_df):
    ...
    (This is your original function, unchanged and correctly cached)
    """
...
# --- NEW CACHE-SAFE FUNCTION FOR LIVE DATA ---
@st.cache_data(ttl=3600) # Cache for 1 hour
def get_live_dividend_yields(tickers):
    """
    This is the "Data Kitchen". It ONLY fetches data and returns a dictionary.
    It does not use any st.* UI elements.
    """
    yields = {}
    if not tickers:
        return {}
...
try:
    ...
    # This is your original logic to get holdings
...
# ... (The rest of your metrics code is unchanged)
...
# --- MODIFIED: Added the Cash Flow tab to the declaration ---
...
with tab_main:
    ...
    try:
        conn = sqlite3.connect(DB_FILE)
        # We now use the `current_holdings_df` calculated earlier
        ...
        if not holdings_summary_df.empty:
            ...
            # --- THIS IS THE FIX ---
            # This is the "Storefront". It calls the kitchen, gets the data, then displays it.
            # 1. Get the list of tickers you own
            all_tickers = holdings_summary_df['instrument'].tolist()
            # 2. Call the cached "kitchen" function to get the data
            ...
            # --- MODIFIED: Added 'dividend_yield' to the list of columns to display ---
            st.dataframe(
                ...
                column_config={
                    ...
                    "dividend_yield": st.column_config.NumberColumn("Dividend Yield", format="%.2f%%"), # <-- And configured its format
    ...
    # ... (The rest of your code for charts and other tabs remains unchanged)
